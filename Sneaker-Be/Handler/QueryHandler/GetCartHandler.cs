...
namespace Sneaker_Be.Handler.QueryHandler
{
    public class GetCartHandler : IRequestHandler<GetCart, ProductFromCartDto>
    {
        ...
        {
            var query = "SELECT p.id, p.name, p.price, p.thumbnail, p.description, p.created_at as CreatedAt, p.updated_at as UpdateAt, p.category_id as CategoryId, p.sale,c.quantity, c.size FROM products p " +
